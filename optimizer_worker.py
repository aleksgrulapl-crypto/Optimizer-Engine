...
# win rate >= 45%, profit factor >= 1.4, strictly positive net profit, low DD.
...
# profit, profit factor >= 1.4, win rate >= 45%, at least 100 trades, and low max
...
def is_strong_candidate(metrics: Dict[str, Any], strong_filters: Dict[str, Any] = None) -> bool:
    """Check for positive profit, PF >= 1.4, win rate >= 45%, 100+ trades,
    and low max drawdown before treating a ticker/timeframe run as complete."""
...
def optimize_ticker(cfg: Dict[str, Any],
    ...
    for search_position, params in _grid_search_entries(
        ...
        for path in intrabar_paths:
            ...
            # Hard filters (defaults: net profit > 0, PF >= 1.4, win rate >= 45%,
            # max drawdown <= 25%, at least 100 trades)
            pf = float(metrics.get("profit_factor", 0.0))
            net = float(metrics.get("net_profit", 0.0))
            wr = float(metrics.get("win_rate", 0.0))
            tc = int(metrics.get("trade_count", 0))
